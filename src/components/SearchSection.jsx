...
const SearchSection = ({ getWeatherData }) => {
   const API_KEY = import.meta.env.VITE_API_KEY;

   const handleSearch = (e) => {
      e.preventDefault();
      const searchValue = e.target.search.value;
      const API_URL = `http://api.weatherapi.com/v1/current.json?key=${API_KEY}&q=${searchValue}`;
      getWeatherData(API_URL);
   }

	return (
		<div className="search-section w-full flex p-4 gap-2">
			<form action="#" className="search-form w-full flex h-[50px] relative" onSubmit={handleSearch}>
